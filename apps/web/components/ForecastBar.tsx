...
export function ForecastBar({ current, forecast }: Props) {
  ...
  return (
    <div className="ach-panel px-4 py-4">
      <p className="mb-3 text-[11px] font-semibold uppercase tracking-widest text-[var(--ach-text-muted)]">{t('pollen.forecast')}</p>
      <div className="grid grid-cols-7 gap-1" role="list">
        {days.map((day, i) => {
          const levelLabel = t(`pollenLevel.${day.overallLevel}` as 'pollenLevel.low');
          return (
          <div key={day.date} className="flex flex-col items-center gap-1.5 rounded-lg bg-[var(--ach-surface-soft)] px-1 py-2" role="listitem" aria-label={`${dayLabel(day.date, i)}: ${t('pollen.title')} ${levelLabel}`}>
            <span className="text-xs font-medium text-[var(--ach-text-secondary)]">{dayLabel(day.date, i)}</span>
            <span className={`w-3 h-3 rounded-full ${DOT[day.overallLevel] ?? 'bg-[var(--ach-line)]'}`} aria-hidden="true" />
            <span className={`text-[10px] text-center leading-tight font-semibold ${LEVEL_TEXT[day.overallLevel] ?? 'text-[var(--ach-text-secondary)]'}`}>
              {levelLabel}
            </span>
          </div>
          );
